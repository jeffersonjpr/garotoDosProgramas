...
//estrutura para ligaçoes entre casas
typedef struct casaligada{
	char nome[5];
	struct casaligada *prox;
}casaligada;

//estrutura do mosquito
typedef struct mosquito{
	//conta o numero de passo
	int Npassos;
	//bool para verificar se ja se moveu quando a funçao mosquitomove() for chamada
	char moved;
	struct mosquito *prox;
}mosquito;
/*
representação das structs
	###############
	# nome = C3   #
	# infected = 1#
	# nlig = 2    #
...
typedef struct casaprincipal{
	...
	char nome[5];
	//numero de casas ligadas,nº de pessoas
...
//a funçao abaixo reseta todas as boleanas dos mosquitos para 0 para indicar que nenhum se moveu ainda
...
void mosquito_move(casas*x){
	...
	while(aux != NULL){
		...
		while(*auxm != NULL){
			...
			(*auxm) -> moved = 1;
			(*auxm) -> Npassos += 1;
			//caso 1: move o mosquito para casa que nao possui mosquito
			if(aux2 -> m == NULL){
				aux2 -> infected = 1; //infectando a casa para onde o mosquito vai
				aux2 -> m = *auxm;
				(*auxm) -> prox = NULL;
				*auxm = auxm2;
			...
			//caso 2: move o mosquito para casa que ja possui mosquito
			aux2 -> infected = 1; //infectando a casa para onde o mosquito vai
			auxm3 = aux2 -> m;
...
//AGENTE ATUAA
//retorna 1 caso acerte ou 0 caso erre
...
int agente_saude(casas *p, int cont){
	...
	//verificando se existem mosquitos.
	if(aux->m != NULL){
...
int main(){
	...
	while(!feof(arq)){
		if(!strcmp(doit,"FIM")){
			...
			//para verificar o surto
			//Caso parecido ao da primeira verificação, no final podem acontecer movimentos ou açoes dos agentes
			nIfectadosTot = containfectado(principal); //conta o numero de infetados apos o mmove
			if(keykey)
				if(nIfectadosTot > nPessoas80porcento){
					chaveSurto = 1; //verifica o surto
