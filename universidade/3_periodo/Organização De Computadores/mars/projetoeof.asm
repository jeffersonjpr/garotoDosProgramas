...
.align 4
	...
	localArquivo: .space 150				# espaço para nome do arquivo ou caminho ate ele
	resultado: .asciiz "resultado.pgm"			# Arquivo utilizado para auxiliar no somatorio
	bbyte: .half 1					# espaço para um byte
	
	vetorDDA: .word 11					# 11 palavras, 1º palavra dda resultado
...
abreDescritores:
	...
	sw $s0, 4($sp)					# guardando registradores					
	# s1 nao estava em uso
	
	add $s0, $a0, $zero				# quantidade de arquivos
	# s0 tem quantidade de arquivos
	jal achaporcentagem				# encontra endereço porcentagem
	add $s1, $v0, $zero				# guardando retorno
	beq $s1, -1, finalizandocomerro			# nome arquivo invalido
	
	# s1 tem o enrenço do porcentagem
	
	addi $s2, $zero, 48				# ascii para zero
	#s2 tem o int igual a 0 ascii
	sb $s2, 0($s1)					# mudando % para 0
	la $a0, localArquivo				# abrindo primeiro arquivo
	la $a1, 0
	jal abriarquivo
	
	blt $v0, $zero, finalizandocomerro		# erro ao abrir o arquivo (nao existe ou nome errado)
...
greri:
	add $t0, $zero, $zero				# boleana para verificaçao do comentario
	...
	loopgreri:
		...
		bne $t2, 35, barraene			# comparando começo comentario
		...
		addi $t3, $t3, -1			# fim de linha alcançado
...
achaporcentagem:
	...
	loopap:
		...
		beq $t1, 37, fimapsucesso		# comparando se e igual a porcentagem
		beq $t1, 0, fimapfalha			# comparando se e igual a NULL
...
# procedimento para printar uma nova linha
printaln:
	li $v0, 4					# code v0 printa string
	la $a0, ln					# string contendo newline
	syscall
	jr $ra

## funcao removebarran: Substitui o \n (proveniente do input do syscall) do final da string por NULLO
## $a0 = ponteiro pro começo da string
removebarran:
	...
	loopremovebn:
		beq $t0, '\n', fimlrbn			# comparaçao para saber se ja chegou no \n
...
finalizandocomerro:
	li $v0, 4					# code syscall print string
